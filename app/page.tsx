...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="section container research-callout"><div><SectionLabel>03 / RESEARCH WORKFLOW</SectionLabel><h2>{uz ? "Ochiq loyiha. O‘lchanadigan natija." : "Open project. Measurable outcomes."}</h2><p>{uz ? "Research workflow: Windows desktop muhitida reproducibility, provenance, testlar va xavfsiz distribution’ni amaliy ishlatish." : "Research workflow: use reproducibility, provenance, testing and safe distribution in a practical Windows desktop environment."}</p></div><div className="research-stat"><span>v1.3.1</span><strong>{uz ? "Windows tayyor" : "Windows ready"}</strong><small>{uz ? "MSIX + EXE fallback" : "MSIX + EXE fallback"}</small></div></section>
  </>;
}
